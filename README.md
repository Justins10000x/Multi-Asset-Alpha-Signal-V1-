Welcome to version 1 of the Alpha generator, current inputs variables include: SPY, QQQ, IWM, VIX, TNX.

Our priority for the next update will aim to address the overfitting issues that's been observed in V1.

Signal recontruction Bugs that were observed in V1 will be updated, execution logic will also be reworked.

The use of T test statistics will be reviewed and appropirate statistical modelling technique chnages will be implemented.

V2 is currently in the works, exploration of new variable inputs and data modeling structures remains to be the priority.


